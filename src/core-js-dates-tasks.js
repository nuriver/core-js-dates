...
function getTime(...date) {
  ...
  const hours =
    `${givenDate.getHours()}`.length > 1
      ? `${givenDate.getHours()}`
      : `0${givenDate.getHours()}`;
  const minutes =
    `${givenDate.getMinutes()}`.length > 1
      ? `${givenDate.getMinutes()}`
      : `0${givenDate.getMinutes()}`;
  const seconds =
    `${givenDate.getSeconds()}`.length > 1
      ? `${givenDate.getSeconds()}`
      : `0${givenDate.getSeconds()}`;
...
function getNextFriday(date) {
  ...
  let daysToFriday;
  if (givenDay < 5) {
    daysToFriday = fridayNum - givenDay;
  } else if (givenDay === 5) {
    daysToFriday = fridayNum + 2;
  } else {
    daysToFriday = fridayNum + 1;
  }
  givenDate.setDate(givenDate.getDate() + daysToFriday);
  return givenDate;
}
/**
...
function getCountDaysInMonth(month, year) {
  const date = new Date(year.toString(), month.toString(), 0);
  return date.getDate();
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  const msBetween = new Date(dateEnd).getTime() - new Date(dateStart).getTime();
  const daysBetween = Math.round(msBetween / (1000 * 3600 * 24)) + 1;
  return daysBetween;
}
...
function isDateInPeriod(date, period) {
  const dateToCheckInMs = new Date(date).getTime();
  const startDateInMs = new Date(period.start).getTime();
  const endDateInMs = new Date(period.end).getTime();
  if (dateToCheckInMs >= startDateInMs && dateToCheckInMs <= endDateInMs) {
    return true;
  }
  return false;
}
...
function formatDate(date) {
  ...
  const hours =
    givenDate.getUTCHours() !== 12 ? givenDate.getUTCHours() % 12 : 12;
  const hourFormat = givenDate.getUTCHours() >= 12 ? 'PM' : 'AM';
  const minutes =
    `${givenDate.getMinutes()}`.length > 1
      ? `${givenDate.getMinutes()}`
      : `0${givenDate.getMinutes()}`;
  const seconds =
    `${givenDate.getSeconds()}`.length > 1
      ? `${givenDate.getSeconds()}`
      : `0${givenDate.getSeconds()}`;
  return `${month}/${day}/${year}, ${hours}:${minutes}:${seconds} ${hourFormat}`;
}
